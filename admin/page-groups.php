...
// Update groups_page to ensure group management
function groups_page(): void {
	...
	if ( $_SERVER['REQUEST_METHOD'] === 'POST' ) {
		...
		if ( isset( $_POST['update_group'] ) ) {
			...
			// Update
			if ( $group_id ) {
				if ( lnb_update_group( $group_id, $group_name ) ) {
		...
		if ( isset( $_POST['delete_group'] ) && $group_id ) {
			if ( $group_id == 1 ) {
				...
				// Check if group is used in links
				...
				if ( $usage_count > 0 ) {
					...
					echo lnb_delete_group( $group_id ) ? "<div class='notice notice-success'><p>Group deleted successfully.</p></div>" : "<div class='notice notice-error'><p>Error deleting group.</p></div>";
				}
			}
		}
	}
    ...
    <div class="wrap">
        ...
        <div class="lnb-card-container">
			...
			// Display groups with update and delete options
			// Display groups with update and delete options
			...
			if ($groups) {
				foreach ($groups as $group) {
					...
					$is_disabled = $group->disabled; // Check if the group is disabled
					?>

                    <div class="lnb-card <?= $is_disabled ? 'lnb-disabled' : ''; ?>">
                        <form method="POST" action="">
                            ...
                            <div class="lnb-card-actions">
                                <button type="submit" name="update_group" class="button button-primary"
									<?= $is_disabled ? 'disabled' : ''; ?>>Update</button>
                                <button type="submit" name="delete_group" class="button button-danger"
                                        onclick="return confirm('Are you sure you want to delete this group?');"
									<?= $cannot_be_deleted || $is_disabled ? 'disabled' : ''; ?>>
                                    Delete
                                </button>
                            </div>
                ...
                <div class="lnb-card-body">
                    <form method="POST" action="">
                        ...
                        <table class="form-table">
                            ...
                            <!-- Name -->
                            <tr>
                                <th><label for="group_name"><span class="required">*&nbsp;</span>Name</label></th>
                                <td><input type="text" name="group_name" id="group_name" class="regular-text"
                                           placeholder="Enter group name" required></td>
                            </tr>
                        </table>
